...
```{r}
set.seed(5)
simulate_pop <- function(N_bw, N_bm, carrying_capacity, pollution_rate, sel_coeff, migra_rate, mut_rate, t_max) {
  ...
  for (i in 1:t_max+1) {
    ...
    pop_new <- simulate_one_gen(pop_new[1],pop_new[2],carrying_capacity, pollution_rate, sel_coeff, migra_rate, mut_rate)
...
output <- simulate_pop(N_bw=1000, N_bm=0,carrying_capacity=1000, pollution_rate=.7, sel_coeff=.8, migra_rate=2, mut_rate=.01, t_max=max_gen)
