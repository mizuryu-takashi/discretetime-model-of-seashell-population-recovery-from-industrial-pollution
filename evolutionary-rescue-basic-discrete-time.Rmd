...
```{r}
simulate_one_gen <- function(N_a, N_ma, N_b, pollution_rate, migra_rate,mut_rate) {
  # draw offspring according to Poisson distribution
  offsp_b <- rpois(1, N_b * (1-pollution_rate))
  ...
  N_b_new <-  offsp_b + migra_a_to_b
  # draw new mutants according to Poisson distribution 
  N_mut <- rpois(1, (N_b_new) * mut_rate)
  return(c(N_a_new, N_b_new))
}
# Test the function
print(simulate_one_gen(N_a =1000, N_ma =0, N_b =1000, pollution_rate =.7, migra_rate =.2, mut_rate =.01))
``` 
